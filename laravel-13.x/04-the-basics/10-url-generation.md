...
Each of these methods may also be accessed via the `URL` [[03-architecture-concepts/04-facades.md|facade]]:
...
Or, via the [[04-the-basics/11-http-session.md|session]], you may access the previous URL as a [fluent URI](#fluent-uri-objects) instance:
...
The `route` helper may be used to generate URLs to [[04-the-basics/01-routing.md#named-routes|named routes]]. Named routes allow you to generate URLs without being coupled to the actual URL defined on the route. Therefore, if the route's URL changes, no changes need to be made to your calls to the `route` function. For example, imagine your application contains a route defined like the following:
...
You will often be generating URLs using the route key (typically the primary key) of [[08-eloquent-orm/01-eloquent-getting-started.md|Eloquent models]]. For this reason, you may pass Eloquent models as parameter values. The `route` helper will automatically extract the model's route key:
...
Instead of validating signed URLs using the incoming request instance, you may assign the `signed` (`Illuminate\Routing\Middleware\ValidateSignature`) [[04-the-basics/02-middleware.md|middleware]] to the route. If the incoming request does not have a valid signature, the middleware will automatically return a `403` HTTP response:
...
For more information on working with fluent URI objects, consult the [[05-digging-deeper/10-helpers.md#uri|URI documentation]].
...
It is cumbersome to always pass the `locale` every time you call the `route` helper. So, you may use the `URL::defaults` method to define a default value for this parameter that will always be applied during the current request. You may wish to call this method from a [[04-the-basics/02-middleware.md#assigning-middleware-to-routes|route middleware]] so that you have access to the current request:
...
Setting URL default values can interfere with Laravel's handling of implicit model bindings. Therefore, you should [[04-the-basics/02-middleware.md#sorting-middleware|prioritize your middleware]] that set URL defaults to be executed before Laravel's own `SubstituteBindings` middleware. You can accomplish this using the `priority` middleware method in your application's `bootstrap/app.php` file:
