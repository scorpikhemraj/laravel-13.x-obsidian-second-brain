...
If you would like these computed values to be added to the array / JSON representations of your model, [[08-eloquent-orm/06-eloquent-serialization.md#appending-values-to-json|you will need to append them]].
...
You may use the `Illuminate\Database\Eloquent\Casts\AsStringable` cast class to cast a model attribute to a [[05-digging-deeper/20-strings.md#fluent-strings-method-list|fluent Illuminate\\Support\\Stringable object]]:
...
To update a single field of a JSON attribute with a more terse syntax, you may [[08-eloquent-orm/01-eloquent-getting-started.md#mass-assignment-json-columns|make the attribute mass assignable]] and use the `->` operator when calling the `update` method:
...
Similarly, Laravel offers an `AsCollection` cast that casts your JSON attribute to a Laravel [[05-digging-deeper/04-collections.md|Collection]] instance:
...
The `of` method may be used to indicate collection items should be mapped into a given class via the collection's [[05-digging-deeper/04-collections.md#method-mapinto|mapInto method]]:
...
If your Eloquent model has a [[07-database/04-migrations.md#column-method-binary|binary type]] `uuid` or `ulid` column in addition to your model's auto-incrementing ID column, you may use the `AsBinary` cast to automatically cast the value to and from its binary representation:
...
When defining a `date` or `datetime` cast, you may also specify the date's format. This format will be used when the [[08-eloquent-orm/06-eloquent-serialization.md|model is serialized to an array or JSON]]:
...
The `encrypted` cast will encrypt a model's attribute value using Laravel's built-in [[06-security/04-encryption.md|encryption]] features. In addition, the `encrypted:array`, `encrypted:collection`, `encrypted:object`, `AsEncryptedArrayObject`, and `AsEncryptedCollection` casts work like their unencrypted counterparts; however, as you might expect, the underlying value is encrypted when stored in your database.
...
When using castables, you may still pass arguments in the `casts` method:

```php
protected function casts(): array
{
    return [
        'address' => Address::class.':arg1,arg2',
    ];
}
```

Castables may also define their own custom casters using the `LazyCaster` class to enable lazy resolution of the casted value. This is useful when the cast class needs access to the model instance, such as the model ID:

```php
use Illuminate\Database\Eloquent\Casts\LazyCaster;

public static function castUsing(): string
{
    return LazyCaster::class;
}
```

When lazy casters are used, the value object will not be instantiated until it is actually accessed, allowing you to access the model instance within the caster: