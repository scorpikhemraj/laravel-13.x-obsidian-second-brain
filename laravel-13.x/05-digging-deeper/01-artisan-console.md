...
Let's take a look at an example command. Note that we are able to request any dependencies we need via the command's `handle` method. The Laravel [[03-architecture-concepts/02-service-container.md|service container]] will automatically inject all dependencies that are type-hinted in this method's signature:
...
In addition to receiving your command's arguments and options, command closures may also type-hint additional dependencies that you would like resolved out of the [[03-architecture-concepts/02-service-container.md|service container]]:
...
When Artisan boots, all the commands in your application will be resolved by the [[03-architecture-concepts/02-service-container.md|service container]] and registered with Artisan.
...
Using the `queue` method on the `Artisan` facade, you may even queue Artisan commands so they are processed in the background by your [[05-digging-deeper/17-queues.md|queue workers]]. Before using this method, make sure you have configured your queue and are running a queue listener:
