...
The best way to understand Laravel's context capabilities is to see it in action using the built-in logging features. To get started, you may [[04-the-basics/02-middleware.md|add information to the context](#capturing-context) using the `Context` facade. In this example, we will use a [middleware]] to add the request URL and a unique trace ID to the context on every incoming request:
