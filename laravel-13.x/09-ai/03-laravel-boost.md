...
| Name | Notes |
|------|-------|
| [Application Info](#application-info) | Read PHP & Laravel versions, database engine, list of ecosystem packages with versions, and Eloquent models |
| [Browser Logs](#browser-logs) | Read logs and errors from the browser |
| [Database Connections](#database-connections) | Inspect available database connections, including the default connection |
| [Database Query](#database-query) | Execute a query against the database |
| [Database Schema](#database-schema) | Read the database schema |
| [Get Absolute URL](#get-absolute-url) | Convert relative path URIs to absolute so agents generate valid URLs |
| [Last Error](#last-error) | Read the last error from the application's log files |
| [Read Log Entries](#read-log-entries) | Read the last N log entries |
| [Search Docs](#search-docs) | Query the Laravel hosted documentation API service to retrieve documentation based on installed packages |
...
To keep your AI guidelines up-to-date, see the [Keeping Boost Resources Updated](#keeping-boost-resources-updated) section.
...
To keep your skills up-to-date, see the [Keeping Boost Resources Updated](#keeping-boost-resources-updated) section.
