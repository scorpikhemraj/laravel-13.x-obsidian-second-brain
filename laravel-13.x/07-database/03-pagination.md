...
In other frameworks, pagination can be very painful. We hope Laravel's approach to pagination will be a breath of fresh air. Laravel's paginator is integrated with the [[07-database/02-query-builder.md|query builder]] and [[08-eloquent-orm/01-eloquent-getting-started.md|Eloquent ORM]] and provides convenient, easy-to-use pagination of database records with zero configuration.
...
There are several ways to paginate items. The simplest is by using the `paginate` method on the [[07-database/02-query-builder.md|query builder]] or an [[08-eloquent-orm/01-eloquent-getting-started.md|Eloquent query]]. The `paginate` method automatically takes care of setting the query's "limit" and "offset" based on the current page being viewed by the user. By default, the current page is detected by the value of the `page` query string argument on the HTTP request. This value is automatically detected by Laravel, and is also automatically inserted into links generated by the paginator.
...
You may also paginate [[08-eloquent-orm/01-eloquent-getting-started.md|Eloquent]] queries. In this example, we will paginate the `App\Models\User` model and indicate that we plan to display 15 records per page. As you can see, the syntax is nearly identical to paginating query builder results:
...
These objects provide several methods that describe the result set. In addition to these helper methods, the paginator instances are iterators and may be looped as an array. So, once you have retrieved the results, you may display the results and render the page links using [[04-the-basics/08-blade-templates.md|Blade]]:
