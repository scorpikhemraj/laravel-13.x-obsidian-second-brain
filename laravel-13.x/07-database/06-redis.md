...
'redis' => [
    ...
    'cache' => [
        'url' => 'tls://[email protected]:6380?database=1',
    ],
...
#### [PhpRedis Serialization](#phpredis-serialization)
...
You may interact with Redis by calling various methods on the `Redis` [[03-architecture-concepts/04-facades.md|facade]]. The `Redis` facade supports dynamic methods, meaning you may call any [Redis command](https://redis.io/commands) on the facade and the command will be passed directly to Redis. In this example, we will call the Redis `GET` command by calling the `get` method on the `Redis` facade:
...
First, let's set up a channel listener using the `subscribe` method. We'll place this method call within an [[05-digging-deeper/01-artisan-console.md|Artisan command]] since calling the `subscribe` method begins a long-running process:
